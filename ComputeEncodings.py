...
class ComputeEncodings:
    ...
    def compute_encodings(self, dataset: Dataset) -> Dict[int, np.ndarray]:
        ...
        dataloader = DataLoader(
            remove_unused_columns(dataset, self.model, True, self._signature_columns),
            shuffle=False, batch_size=self.args.train_batch_size, collate_fn=self.data_collator)
        ...
        for batch in dataloader:
            ...
            database = compute_layer_representations(
                ...
                self.layers_to_save, self.poolers, database
            )
    ...
    def save_encodings(self, database: Dict[int, np.ndarray], path: str):
        ...
        os.makedirs(path)
        save_database_after_stacking(
            self.layers_to_save, path, database
        )
