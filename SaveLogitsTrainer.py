...
class SaveLogitsTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            if isinstance(outputs, dict) and "loss" not in outputs:
                raise ValueError(
                    "The model did not return a loss from the inputs, only the"
                    f" following keys: {','.join(outputs.keys())}. For"
                    " reference, the inputs it received are"
                    f" {','.join(inputs.keys())}."
                )
            ...
            if has_labels:
                ...
                with self.compute_loss_context_manager():
                    ...
                    if model.training or self.save_reps_path is None:
                        ...
                        outputs = model(
                            **inputs,
                            output_hidden_states=self.save_reps_path
                            is not None,
                        )
                    if self.save_reps_path is not None:
                        hidden_states = get_hidden_states(
                        ...
                        # del hidden_states
                        attention_mask = (
                            inputs.get("attention_mask", None).detach().cpu()
                        )
                        self.database = compute_layer_representations(
                            self.model.config.is_encoder_decoder,
                            hidden_states,
    ...
    def _create_save_files(self, prefix: str):
        if self.save_logits:
            self.save_logits_path = os.path.join(
                self.args.output_dir, f"{prefix}_logits.txt"
            )
