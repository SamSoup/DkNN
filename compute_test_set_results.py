...
for dataset in tqdm(DATASETS, desc="datasets"):
    ...
    whitebox_preds = pd.read_pickle(
        os.path.join(
            WORK_DIR, "data", dataset, f"{dataset}_wrapper_box_predictions.pkl"
        )
    )
    # result file layout:
    ...
    for model in tqdm(MODELS, desc="models"):
        for seed in SEEDS:
            ...
            for clf in tqdm(classifiers, desc="original+whiteboxes"):
                if clf == "original":
                    preds = np.array(
                        load_predictions(WORK_DIR, dataset, model_full)
                    )
                else:
                    preds = whitebox_preds.loc[model][clf]
                metrics = compute_metrics(
                    y_test, preds, prefix="test", is_multiclass=is_multiclass
                )
                for metric in METRICS:
                    results.loc[clf][model][metric] = metrics[f"test_{metric}"]
    results.to_pickle(
        os.path.join(
            WORK_DIR, "data", dataset, f"{dataset}_test_set_results.pkl"
        )
    )
